...
const EventsAdmin = (function () {
    ...
    function wire() {
        ...
        $('evAdd').addEventListener('click', async () => {
            ...
            const conflicts = conflictLabels(occupants, 'event');
            if (conflicts.length && !confirm(
                date + ' already has ' + conflicts.join(' and ') + ' scheduled. Add this event anyway?'
            )) return;
    ...
    // conflict warning below - same-type occupancy (another event that
    // day) isn't a conflict, so excludeType is left out of the result.
