-- Distinguish "no handicap yet" from "handicap of 0" (a shooter performing
-- so well their handicap rounds to zero). shooter_handicap() now returns
-- NULL when the shooter hasn't fired the minimum 3 season matches required
-- (see 2026-08-01-handicap-min-season-matches.sql), instead of 0. Callers
-- display NULL as "N/A"; handicaps_for() already passes NULL straight
-- through, and match.js's `handicapMap[id] || 0` still treats it as no
